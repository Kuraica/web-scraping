...
<div class="container">
    ...
    <h3>Enter Processing Order</h3>

    <div class="mb-3">
        <input type="text" name="extension_id" id="ext-id" class="form-control" placeholder="Enter Extension ID">
    </div>

    <!-- Order (Processed By) input field -->
    <div class="mb-3">
        <input type="text" name="order" id="order" class="form-control" placeholder="Enter Processing Order">
    </div>

    <button class="btn btn-primary btn-custom" onclick="startScraping()">Start Agency Scraping</button>
    <button class="btn btn-danger btn-custom" onclick="stopScraping()">Stop Agency Scraping</button>
    <button class="btn btn-success btn-custom" onclick="continueScraping()">Continue Agency Scraping</button>
</div>
...
<script>
    ...
    function startScraping() {
        ...
        if (!order) {
            alert('Please enter a valid Processing Order.');
            return;
        }

        console.warn('Starting agency scraping with Extension ID:', extensionId);
        console.log(chrome.runtime);
        // Send message to the extension to start agency scraping
        chrome.runtime.sendMessage(extensionId, { action: 'startAgencyScraping', order: order }, function (response) {
    ...
    function continueScraping() {
        const extensionId = document.getElementById('ext-id').value.trim();
        const order = document.getElementById('order').value.trim(); // New field for processed_by/order
        ...
        fetch('/api/continue-scraping/' + order, {
            method: 'GET',
            headers: {
